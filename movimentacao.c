...
int movimentoPersonagem(char m[MAPA_ALTURA][MAPA_LARGURA], Boneco *bombeiro)
{
    ...
    bool podeAndar = (tempoAtual - bombeiro->tempoUltimoMovimento >= bombeiro->velocidade);   // calcula a velocidade
    bool podePular = (tempoAtual - bombeiro->tempoUltimoPulo >= bombeiro->velocidade + 0.15); // calcula o delay para o proximo pulo
    if (bombeiro->estaPulando)
    {
        if (tempoAtual >= bombeiro->tempoInicioPulo + 0.35f)
        { // se passou o tempo do pulo começa a cair
            if (m[y + 1][x] != 'Z')
            { // se nao for um chao pode descer
        ...
        {
            bombeiro->posicao.y = y; // se ainda tiver dentro do tempo de pulo não faz nada
        }
    }
    ...
    else if (IsKeyDown(KEY_SPACE))
    {
        ...
        { // verifica limite do mapa, se está em bloco válido para pular
    ...
    if (podeAndar)
    {
        ...
        { // verifica os limites da matriz e se tem suporte embaixo ou se está pulando
        ...
        else if (IsKeyDown('A') || IsKeyDown(KEY_LEFT))
        {
            ...
            { // verifica os limites do mapa, se está em um suporte ou se ta pulando
                bombeiro->posicao.x = x - 1;
                bombeiro->tempoUltimoMovimento = tempoAtual;
                x = x - 1;
            }
        ...
        else if (!bombeiro->estaPulando && (IsKeyDown('S') || IsKeyDown(KEY_DOWN)))
        {
            ...
            { // verifica se nao vai passar da borda e se o bloco é o inicio da descida ou uma escada
                bombeiro->posicao.y = y + 1;
                bombeiro->tempoUltimoMovimento = tempoAtual;
            }
        }
    ...
    if (IsKeyPressed(KEY_ENTER))
    {
        if (m[y][x] == 'F' || m[y][x] == 'L')
        {
            return 1; // passa de fase
        }
    }

    if (m[y][x] == 'E')
    { // ativa o especial, remocve o desenho do especial do mapa para nao pegar de novo
        m[y][x] = '.';
    ...
    {
        bombeiro->especial = false; // desativa o mopo especial
    }
    return 0;
}
...
void initMonstro(Monstro monstros[], int *numMonstros, char m[MAPA_ALTURA][MAPA_LARGURA], float blocoTamanho)
{
    *numMonstros = 0;
    ...
    { // Percorre o mapa para encontrar os monstros
        for (int j = 0; j < MAPA_LARGURA; j++)
        {
            if (m[i][j] == 'M')
            { // Se encontrar um monstro, inicializa suas propriedades
...
void movimentoMonstro(Monstro *monstro, char m[MAPA_ALTURA][MAPA_LARGURA], float blocoTamanho)
{
    ...
    // calcula as posicoes do monstro para coordenadas da matriz
    int gridX = (int)(monstro->x / blocoTamanho);
    int gridY = (int)(monstro->y / blocoTamanho);

    // Verifica se o próximo movimento é possível
    if (monstro->direcao == 1)
    { // Movendo para a direita
        if (gridX + 1 < MAPA_LARGURA && (m[gridY + 1][gridX + 1] == 'Z' || m[gridY + 1][gridX + 1] == 'H'))
        {                                                     // Verifica se o bloco à direita abaixo é um chão ou uma escada
    ...
    { // Movendo para a esquerda
        if (gridX - 1 >= 0 && (m[gridY + 1][gridX - 1] == 'Z' || m[gridY + 1][gridX - 1] == 'H'))
        {                                                     // Verifica se o bloco à esquerda abaixo é um chão ou uma escada
...
{ // Verifica se o monstro colidiu com o personagem
    for (int y = 0; y < MAPA_ALTURA; y++)
    {
        for (int x = 0; x < MAPA_LARGURA; x++)
        {
            if (y == b->posicao.y && x == b->posicao.x)
            { // se as coordenadas x e y baterem com a posicao do jogador
                int monstroGridX = (int)(monstro->x / blocoTamanho);
                int monstroGridY = (int)(monstro->y / blocoTamanho); // converte os pixels do monstro p matriz
...
{
    // Calculos para o desenho do monstro
